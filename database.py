...
def init_db():
    ...
    # Удаляем таблицу users, если она есть
    cursor.execute("DROP TABLE IF EXISTS users;")
    conn.commit()

    # Создаем таблицу users заново с UNIQUE для telegram_id
    cursor.execute("""
    ...
    # Удаляем таблицу articles, если она есть
    cursor.execute("DROP TABLE IF EXISTS articles;")
    conn.commit()

    # Создаем таблицу статей заново
    cursor.execute("""
...
if __name__ == "__main__":
    ...
    load_articles()  # Загружаем статьи